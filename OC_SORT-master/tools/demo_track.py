...
from yolox.utils.visualize import plot_tracking_kf
...
def gt_demo(vis_folder,current_time):
    ...
    for frame_id, img_path in enumerate(files, 1):
        gt_path=args.path.rstrip('img1')+"gt\gt.txt"
        f= open(gt_path, "r")
        ...
        img_info=[]
        img_info.append(800)
        img_info.append(1440)
        for line in f:
            linedata=line.split(',')
            if int(linedata[0])==frame_id:  
                ...
                cnt+=1
        f.close()
        online_targets,tlwh_kf = tracker.update(dets, img_info, exp.test_size)
        online_tlwhs = []
        online_ids = []
        ...
        online_im = plot_tracking_kf(
            raw_img, online_tlwhs, tlwh_kf, online_ids, frame_id=frame_id, fps=0
        )
...
if __name__ == "__main__":
    ...
    current_time = time.localtime()
    gt_demo(args.out_path,current_time)
